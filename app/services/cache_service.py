...
class CacheService:
        ...
        if storage_backend is None:
            ...
            if backend_type == 's3':
                try:
                    ...
                    logger.info(f"Using the storage bucket: {settings.S3_CACHE_BUCKET}")
                ...
                self.storage = LocalStorageBackend()
                logger.info(f"Using the local storage dir: {settings.CACHE_DIR}")
            ...
            logger.info(f"Using custom storage backend: {type(storage_backend)}")
    ...
    def compute_document_id(self, file_path: Path) -> str:
        ...
        # creates a SHA-256 hash obj 

        with open(file_path, "rb") as f:
            # reads file in raw bytes useful because we will use the bytes when updating
            for chunk in iter(lambda: f.read(8192), b''): 
                # reads the file 8192 bytes at a time instead of loading entire file into memory (b is the stopping condition)
                sha256.update(chunk)
                # we are progressively feeding data into it using sha256.update()

        doc_id = sha256.hexdigest()
        # returns computed SHA-256 hash value as a readable string for 64 hexadecimal characters 
    ...
    def save_document(self, doc_id: str, file_path: Path, file_extension: str) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to save documents {doc_id}: {e}")
            raise 
    ...
    def save_chunks_and_embeddings(
        ...
        if len(chunks) != len(embeddings):
            raise ValueError(f"Chunk/embeddings mismatch: {len(chunks)} chunks, {len(embeddings)} embeddings")
        ...
        try:
            ...
            logger.info(f"Cached {len(chunks)} for {doc_id}: (type: {file_extension})")
